...
pub struct Exchange<'info> {
    ...
    #[account(mut)]
    pub receiver_sender_account: AccountInfo<'info>,
...
pub fn handler_exchange(ctx: Context<Exchange>) -> Result<()> {
    ...
    let swap_value = ctx.accounts.escrow_state.swap_value;
    ...
    match trade_type {
        ...
        Ok(TradeType::TokenToken) => {
            // convert `receiver_sender_account` to TokenAccount
            let partner_token_accounts: (TokenAccountType, TokenAccountType) = (
                Account::try_from(&ctx.accounts.receiver_sender_account),
            ...
            match partner_token_accounts {
                (Ok(partner_send_token_account), Ok(partner_receive_token_account)) => {
                    ...
                    // make sure `receiver_sender_account` is associated token account with `creator_receive_token_mint`
            ...
            ctx.accounts.forward_tokens_to_receiver(swap_value)?;
        ...
        Ok(TradeType::SolToken) => {
            ...
            ctx.accounts.transfer_to_partner_native(swap_value)?;
...
impl<'info> Exchange<'info> {
    ...
    fn forward_tokens_to_receiver(&self, amount: u64) -> Result<()> {
        let creator_key = self.creator.key();
        let order_id_bytes = self.escrow_state.order_id.to_le_bytes();
    ...
    fn transfer_to_creator_token(&self, amount: u64) -> Result<()> {
        // transfer token partner -> creator
        transfer_token_to_account(
            self.receiver_sender_account.to_account_info(),
